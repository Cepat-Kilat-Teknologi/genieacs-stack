...
> For deployment with NBI API authentication, see: `examples/kubernetes/overlays/nbi-auth/`
>
...
Edit resource limits in `genieacs-deployment.yaml` and `../../base/mongodb-deployment.yaml`:
...
Edit storage size in `../../base/genieacs-pvc.yaml` and `../../base/mongodb-pvc.yaml`:
...
└── overlays/
    └── default/
        ├── kustomization.yaml       # Overlay Kustomize configuration
...
- For NBI API authentication, use: `examples/kubernetes/overlays/nbi-auth/`
